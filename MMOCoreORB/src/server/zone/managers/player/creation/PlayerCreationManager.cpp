				...
				if (accountPermissionLevel < 9) {
					try {
						...
						if (res != NULL && res->next()) {
							...
							if (timeVal.miliDifference() < 1200) {
								ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character every 20 minutes. Repeat attempts prior to 20 minutes elapsing will reset the timer.", 0x0);
								client->sendMessage(errMsg);
					...
					if (lastCreatedCharacter.containsKey(accID)) {
						...
						if (lastCreatedTime.miliDifference() < 1200) {
							ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character every 20 minutes. Repeat attempts prior to 20 minutes elapsing will reset the timer.", 0x0);
							client->sendMessage(errMsg);
